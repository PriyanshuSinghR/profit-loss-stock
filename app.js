...
function clickHandler() {
  ...
  if (
    Number(initialPrice.value < 1) ||
    Number(stock.value) < 1 ||
    Number(currentPrice.value) < 1
  ) {
    ...
    if (Number(initialPrice.value) < Number(currentPrice.value)) {
      computeProfit(
        Number(initialPrice.value) * Number(stock.value),
        Number(currentPrice.value) * Number(stock.value),
      );
...
function computeProfit(costPrice, sellingPrice) {
  ...
  displayMessage(
    `Your Profit is ${profit} and the profit percentage is ${profitPercent.toFixed(
      2,
    )}%.
    🥳`,
  );
...
function computeLoss(costPrice, sellingPrice) {
  ...
  displayMessage(
    `Your loss is ${loss} and the loss percentage is ${lossPercent.toFixed(2)}%.
    😔`,
  );
